...
card_info = {
    ...
    'start-1436':  ['start',  'x',   'The Juven Isles'],

    '0000a':    ['3rw',   's-t',   'Fraj Isle'],            # isle
    '0000b':    ['xw',    't-f',   'Argh Isle'],            # isle
    '0000c':    ['xw',    'f-s',   'Tact Isle'],            # isle

    '0014a':    ['rw',    'f-t',   'Point Exter'],          # point
    '0014b':    ['xw',    'f-s',   'Exclamation Point'],    # point
    '0014c':    ['xw',    's-t',   'Mid Point'],            # point

    #'0016a':    ['pl',    'pt-s',  'Davenport'],            # port
    '0016a':    ['pl',    't-s-x',  'Davenport'],           # port
    #'0016b':    ['pw',    'ps-f',  'Bloodsport'],           # port
    '0016b':    ['pw',    's-f-x',  'Bloodsport'],          # port
    '0016c':    ['rdl',   'f-t-x', 'Dissa Point'],          # point

    '0034a':    ['pl',    'f-t-x', 'Counter Point'],        # point
    #'0034b':    ['pw',    'ps-t',  'Port Ative'],           # port
    '0034b':    ['pw',    's-t-x',  'Port Ative'],          # port
    '0034c':    ['rdl',   'f-s-x', 'Miss Isle'],            # isle

    #'0036a':    ['pw',    'pf-s',  'Purr Port'],            # port
    '0036a':    ['pw',    'f-s-x',  'Purr Port'],           # port
    '0036b':    ['rl',    'f-t-x', 'Data Point'],           # point
    '0036c':    ['rw',    's-t-x', 'Rept Isles'],           # isle
    ...
    '0176b':    ['pw',    't-C-x',  'Ruby Port'],           # port
    '0176c':    ['rl',    'H-s-x', 'Imbess Isle'],          # isle

    #'0374a':    ['pl',    'pH-t',  'Port Starboard'],       # port
    '0374a':    ['pl',    'H-t-x',  'Port Starboard'],      # port
    #'0374b':    ['pw',    'pf-B',  'Spoilsport'],           # port
    '0374b':    ['pw',    'f-B-x',  'Spoilsport'],          # port
    '0374c':    ['rw',    's-C-x', 'Croco d’Isle'],         # isle

    #'0376a':    ['pl',    'pB-f',  'Port Entous'],          # port
    '0376a':    ['pl',    'B-f-x',  'Port Entous'],         # port
    #'0376b':    ['pw',    'ps-H',  'Port O’Bello'],         # port
    '0376b':    ['pw',    's-H-x',  'Port O’Bello'],        # port
    '0376c':    ['rdw',   't-C-x', 'Bomb Bay'],             # bay

    '1414a':    ['rw',    'f-t',   'Kidneystone Pass'],     # pass
    '1414b':    ['xw',    's-t',   'None-shall Pass'],      # pass

    #'1416a':    ['pl',    'pC-f',  'Port-au-Potée'],        # port
    '1416a':    ['pl',    'C-f-x',  'Port-au-Potée'],       # port
    #'1416b':    ['pw',    'pt-B',  'Watersport'],           # port
    '1416b':    ['pw',    't-B-x',  'Watersport'],          # port
    '1416c':    ['rdl',   'H-s',   'Tress Pass'],           # pass

    #'1434a':    ['pl',    'pf-s',  'Tele Port'],            # port
    ...
    '1436a':    ['pl',    'B-t-x',  'Vintage Port'],        # port
    #'1436b':    ['pw',    'pf-H',  'Port Able'],            # port
    '1436b':    ['pw',    'f-H-x',  'Port Able'],           # port
    '1436c':    ['2rdw',  's-C-x', 'Percent Isle'],         # isle
    '1436d':    ['rw',    'f-C',   'Backstage Pass'],       # pass

    '1616a':    ['2rdl',  'B-s-x', 'Crooked Strait'],       # strait
    '1616b':    ['2rdw',  't-H-x', 'Lulla Bay'],            # bay

    #'1634a':    ['pl',    'pB-f',  'Serial Port'],          # port
    '1634a':    ['pl',    'B-f-x',  'Serial Port'],         # port
    '1634b':    ['pw',    's-C-x', 'Landing Bay'],          # bay
    '1634c':    ['2rdl',  'H-t-x', 'Inappropriate Pass'],   # pass
    '1634d':    ['rl',    'C-t',   'Letme Inlet'],          # inlet

    #'1636a':    ['pl',    'pC-t',  'Sally Port'],           # port
    ...
    '1774b':    ['rl',    'B-t-x', 'Fewt Isle'],            # isle
    '1774c':    ['rw',    'f-H',   'Duct Isle'],            # isle

    #'1776a':    ['pl',    'pB-C',  'Port End'],             # port
    '1776a':    ['pl',    'B-C-x',  'Port End'],            # port
    #'1776b':    ['pw',    'pH-C',  'Data Port'],            # port
    '1776b':    ['pw',    'H-C-x',  'Data Port'],           # port
    '1776c':    ['rdw',   'B-H-x', 'Project Isle'],         # isle

    '3434a':    ['2rdl',  'H-f-x', 'Forward Pass'],         # pass
    '3434b':    ['2rdw',  's-B-x', 'Infant Isle'],          # isle

    #'3436a':    ['pl',    'pC-B',  'Port Cullis'],          # port
    '3436a':    ['pl',    'C-B-x',  'Port Cullis'],         # port
    '3436b':    ['pw',    'H-B-x', 'Cove Ert'],             # cove
    #'3436c':    ['rdw',   'pH-C',  'Ex Port'],              # port
    ...
    '3776a':    ['rl',    'B-H',   'Stare Isle'],           # isle
    '3776b':    ['xl',    'B-C',   'Cove Arient'],          # cove
    '3776c':    ['xl',    'C-H',   'Crybey Bay'],           # bay

    '7777a':    ['3rl',   'B-C',   'Down Hill'],            # hill
    '7777b':    ['xl',    'C-H',   'Dist Hill'],            # hill
    '7777c':    ['3rl',   'B-H',   'Domis Isle'],           # isle
}
...
class IslandsGen(object):
    def __init__(self, options):
        ...
        self.draw_resource = {
            ...
            'smallport': self.draw_small_port,
        }
    ...
    def draw_card(self, name, data):
        ...
        self.svg.add_def(SvgDefGradientStops('gradientStopsFish', [[0, 'ff5555', 1], [1, 'ffffff', 1]]))
        self.svg.add_def(SvgDefGradientStops('gradientStopsCoconut', [[0, 'ffffff', 1], [1, 'c8c8c8', 0]]))
        self.svg.add_def(SvgDefGradientStops('gradientStopsFlower', [[0, 'b16dff', 1], [1, 'e7d2ff', 1]]))
        # gradient linear xlink, x1, y1, x2, y2, transform
        self.svg.add_def(SvgDefGradientLinear('gradientFish', 'gradientStopsFish', 178.78909, -114.78283, 180.4073, -103.78283, 'translate(-178.76256,1.0606602)'))
        # gradient radial xlink, cx, cy, fx, fy, r, transform
        self.svg.add_def(SvgDefGradientRadial('gradientCoconut', 'gradientStopsCoconut', 1.9440963, -60.612652, 1.9440963, -60.612652, 6.7232941, 'matrix(1.691364,-0.31394812,0.29762395,1.603419,17.826033,-7.042221)'))
        self.svg.add_def(SvgDefGradientRadial('gradientFlower', 'gradientStopsFlower', 60.884877, -69.65275, 60.884877, -69.65275, 15.83531, 'matrix(0.8413905,0,0,0.87120012,-50.722984,-48.679159)'))
    ...
    def draw_guide_route(self, percent):
        ...
        x += self.bleed
        self.svg.path('m %g,0 0,%g' % (x, -self.guide_height), {'style': self.route_style})
    ...
    def draw_guide(self, id, pattern, transform):
        ...
        self.svg.start_group({'id': 'guide%d-%d' % (id, pattern), 'transform': transform})
        
        if pattern == 0:
            self.svg.rect(0, -height, width, height, {'style': deep_water_style})
            self.draw_guide_route(0.5)
            
        if pattern == 1:
            self.svg.rect(width - land1, -height, land1, height, {'style': land_style})
            self.svg.rect(width - grass1, -height, grass1, height, {'style': grass_style})
            self.svg.rect(width - forest1, -height, forest1, height, {'style': forest_style})
            self.svg.rect(0, -height, shallow_water2, height, {'style': shallow_water_style})
            self.svg.rect(0, -height, water2, height, {'style': water_style})
            self.svg.rect(0, -height, deep_water2, height, {'style': deep_water_style})
            self.draw_guide_route(1.0/3.0)

        if pattern == 3:
            self.svg.rect(width - land2, -height, land2, height, {'style': land_style})
            self.svg.rect(width - grass2, -height, grass2, height, {'style': grass_style})
            forest_offset = width - forest2a
            self.svg.rect(forest_offset, -height, forest2a, height, {'style': forest_style})
            forest_offset -= forest2_space + forest2b
            self.svg.rect(forest_offset, -height, forest2b, height, {'style': forest_style})
            forest_offset -= forest2_space + forest2c
            self.svg.rect(forest_offset, -height, forest2c, height, {'style': forest_style})
            self.svg.rect(0, -height, shallow_water1, height, {'style': shallow_water_style})
            self.svg.rect(0, -height, water1, height, {'style': water_style})
            self.svg.rect(0, -height, deep_water1, height, {'style': deep_water_style})
            self.draw_guide_route(2.0/3.0)

        if pattern == 4:
            self.svg.rect(0, -height, land1, height, {'style': land_style})
            self.svg.rect(0, -height, grass1, height, {'style': grass_style})
            self.svg.rect(0, -height, forest1, height, {'style': forest_style})
            self.svg.rect(width - shallow_water2, -height, shallow_water2, height, {'style': shallow_water_style})
            self.svg.rect(width - water2, -height, water2, height, {'style': water_style})
            self.svg.rect(width - deep_water2, -height, deep_water2, height, {'style': deep_water_style})
            self.draw_guide_route(2.0/3.0)

        if pattern == 6:
            self.svg.rect(0, -height, land2, height, {'style': land_style})
            self.svg.rect(0, -height, grass2, height, {'style': grass_style})
            forest_offset = 0
            self.svg.rect(forest_offset, -height, forest2a, height, {'style': forest_style})
            ...
            forest_offset += forest2_space + forest2b
            self.svg.rect(forest_offset, -height, forest2c, height, {'style': forest_style})
            self.svg.rect(width - shallow_water1, -height, shallow_water1, height, {'style': shallow_water_style})
            self.svg.rect(width - water1, -height, water1, height, {'style': water_style})
            self.svg.rect(width - deep_water1, -height, deep_water1, height, {'style': deep_water_style})
            self.draw_guide_route(1.0/3.0)

        if pattern == 7:
            self.svg.rect(0, -height, width, height, {'style': grass_style})
            self.svg.rect(0, -height, forest3, height, {'style': forest_style})
            self.svg.rect(width - forest3, -height, forest3, height, {'style': forest_style})
            self.draw_guide_route(0.5)
    ...
    def draw_turtle(self, id='', transform=''):
        ...
        paths = [
            ...
            # Right Rear Flipper
            'm 0.519205,-98.947277 c -1.160888,-0.35295 2.907731,-2.472353 3.949394,-1.850123 1.642844,0.981343 2.297204,4.963396 0.372373,6.157721 -0.745218,0.462395 -0.785725,-3.232509 -4.321767,-4.307598 z',
            # Right Front Flipper
            'm 7.618715,-114.43269 c 1.934345,-0.44611 4.329746,-0.0993 5.914203,1.00302 2.013049,1.40054 2.917028,5.29708 1.897177,7.52728 -0.282844,0.61852 -3.514643,-5.32749 -6.150465,-4.42389 -1.839455,0.63059 -2.536626,0.15372 -3.169919,-1.68481 -0.27477,-0.79769 0.686895,-2.232 1.509004,-2.4216 z',
            # Left Front Flipper
            'm -13.605402,-118.51893 c 1.434388,-2.65109 5.159923,-3.50053 7.13722,-3.29637 1.742717,0.17994 3.474416,1.88524 3.953288,3.57051 0.312727,1.10056 -0.218032,2.84568 -1.315095,3.17046 -1.558665,0.46144 -1.358953,-1.39147 -2.644475,-2.38636 -2.809563,-2.17437 -7.671044,-0.06 -7.130938,-1.05824 z',
            # Head
            ...
            # Tail
            'm -4.37156,-100.05265 c -1.568711,0.833903 -1.519982,4.156989 -0.84283,4.365801 0.803045,0.247633 2.774585,-2.210096 2.390927,-3.815868 -0.20218,-0.846213 -0.779871,-0.958313 -1.548097,-0.549933 z',
            # Shell
    ...
    def draw_squid(self, id='', transform=''):
        ...
        paths = [
            ...
            'm -3.796875,-104.89258 c -0.9058673,0.74517 -1.9188533,1.2608 -2.7324219,1.16406 -0.2030019,0.0833 -0.4030106,0.17309 -0.5839843,0.29102 -0.166691,0.11469 -0.3464295,0.21948 -0.5195313,0.33008 1.7036649,0.94245 3.731315,0.084 4.8828125,-1.03906 -0.3560506,-0.22586 -0.7057338,-0.47583 -1.046875,-0.7461 z',
        ...
        # Eyes
        self.svg.ellipse(0.28214264, -107.8961, 0.50823301, 0.86178637, {'style': style})
        self.svg.ellipse(-1.629467, -107.9845, 0.50823301, 0.86178637, {'style': style})
        ...
        paths = [
            'm 16.079505,-112.80178 c 0,0 -0.534036,-1.98412 -1.352253,-2.375 -1.083027,-0.51738 -2.029029,0.0701 -1.654029,0.82009 0.375,0.75 2.381282,1.33616 3.006282,1.55491 z',
            'm -15.544194,-112.67678 c 0,0 -0.104921,-1.79881 0.731694,-2.49777 0.96875,-0.80936 1.875,-0.15625 1.59375,0.53125 -0.28125,0.6875 -1.825444,1.62277 -2.325444,1.96652 z',
        ]
        for path in paths:
            self.svg.path(path, {'style': style})
    ...
    def draw_fish(self, id='', transform=''):
        ...
        self.svg.ellipse(35.346939, -105.81317, 0.85538602, 1.0929933, {'style': style, 'transform': 'matrix(0.97605309,-0.21753245,0.21753245,0.97605309,0,0)'})
    ...
    def draw_coconut(self, id='', transform=''):
        ...
        self.svg.ellipse(-120.42445, -1.1047294, 0.54040664, 0.75130093, {'style':style, 'transform': 'matrix(0.04286687,0.99908079,-0.99908079,0.04286687,0,0)'})
        self.svg.ellipse(-113.37407, -41.165329, 0.53094649, 0.66291261, {'style':style, 'transform': 'matrix(0.35065914,0.93650316,-0.93650316,0.35065914,0,0)'})
        self.svg.ellipse(-115.40491, 28.144659, 0.75130093, 0.578731, {'style':style, 'transform': 'matrix(-0.21617976,0.97635358,-0.97635358,-0.21617976,0,0)'})

        # Broken coconut
        path = 'm 12.470061,-104.40917 c -1.053625,2.46314 -2.7571896,5.66371 -5.3992166,6.959878 -1.8348159,0.900154 -4.8858287,1.355265 -7.29203855,0.574524 -1.47973495,-0.480128 -3.26695685,-2.153861 -3.88908725,-3.579732 -0.9764548,-2.23794 -1.1678327,-4.60408 -0.6812181,-6.72953 0.2499469,-1.09173 1.186875,-3.22454 1.8151019,-4.00521'
        self.svg.path(path, {'style': brown_style})
        path = 'm -4.1031709,-106.48916 c 0.1858925,1.26956 0.9057567,2.15876 1.8143573,3.06473 0.7258639,0.72377 1.43443306,1.23318 2.38813597,1.53034 0.21168639,0.36954 0.58409309,0.82966 0.71913043,1.00496 0.6211732,0.30789 1.2701423,0.41934 1.7697881,0.53351 1.2859005,0.29381 3.2051814,0.30928 4.487449,0 1.4766975,-0.35618 2.9730411,-1.01232 4.0560481,-2.07749 0.799348,-0.78619 1.474351,-1.85192 1.582847,-2.96784 0.19684,-2.02457 0.0097,-3.62122 -1.250374,-5.21803 -0.765762,-0.24844 -1.101356,-0.47744 -2.0001007,-0.72008 -0.7209049,-0.88161 -1.2999824,-1.55958 -2.1970327,-2.39292 -1.8286746,-0.73618 -4.3613695,-0.80559 -6.0281383,-0.27571 -1.72586214,0.54868 -3.34971,1.59922 -4.0200406,2.49515 -1.4298123,1.911 -1.5302056,3.6019 -1.3220696,5.02338 z'
        self.svg.path(path, {'style': white_style})
    ...
    def draw_flower(self, id='', transform=''):
        ...
        self.svg.circle(60.884876, -69.652748, 9.6685247, {'style': style, 'transform': 'matrix(0.8413905,0,0,0.8413905,-50.722984,-50.755482)'})
        ...
        self.svg.circle(60.884876, -69.652748, 4.5872564, {'style': style, 'transform': 'matrix(0.8413905,0,0,0.8413905,-50.722984,-50.755482)'})
        ...
        paths = [
            ...
            'm -7.806877,-115.72644 c 1.94457,0.58391 6.643912,3.73434 8.129359,5.1037 0.836444,0.77108 1.096168,1.65836 0.65443,1.94368 -0.441738,0.28531 -1.060198,0.0838 -1.882086,-0.97545 -0.735118,-0.94742 -4.303093,-4.33376 -7.586861,-5.69825 -0.596704,-0.24795 -0.206304,-0.64136 0.685158,-0.37368 z',
        ]
        for path in paths:
            self.svg.path(path, {'style': style})
        ...
        for circle in circles:
            self.svg.circle(circle[0], circle[1], 0.57845598, {'style': style, 'transform': 'matrix(0.99754986,0.06995909,-0.06995909,0.99754986,0,0)'})
    ...
    def draw_banana(self, id='', transform=''):
        ...
        bunch = [
            {'outline': banana1, 'line': line1},
            {'outline': banana2, 'line': line2},
            {'outline': banana3, 'line': line3},
        ]
        
        # Thick outline bananas drawn behind the bunch.
        ...
        for banana in bunch:
            self.svg.path(banana['outline'], {'style': thick_style})
        ...
        fill_style = 'color:#000000;clip-rule:nonzero;display:inline;overflow:visible;visibility:visible;opacity:1;isolation:auto;mix-blend-mode:normal;color-interpolation:sRGB;color-interpolation-filters:linearRGB;solid-color:#000000;solid-opacity:1;fill:#f7ff7d;fill-opacity:1;fill-rule:evenodd;stroke:#000000;stroke-width:0.72000003;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-dashoffset:0;stroke-opacity:1;color-rendering:auto;image-rendering:auto;shape-rendering:auto;text-rendering:auto;enable-background:accumulate'
        for banana in bunch:
            self.svg.path(banana['outline'], {'style': fill_style})
            self.svg.path(banana['line'], {'style': line_style})
    ...
    def draw_pirate(self, id='', transform=''):
        ...
        self.svg.path(path, {'style': style, 'transform': "scale(-1,-1)"})
        ...
        # White background
        path = "m 95.61696,289.40928 c -0.28542,-0.70119 -0.64431,-0.0511 -1.28032,-1.5943 -0.1555,-0.3773 0.0533,-1.26257 -0.20606,-1.97221 -1.08563,-2.97045 -0.70376,-4.38729 -0.13829,-5.43857 0.90077,-1.67462 2.37389,-2.68893 4.18898,-3.25565 1.68855,-0.52721 3.7114,-0.48187 5.36931,0.13497 1.87949,0.69928 2.96812,1.82798 3.83709,3.63528 0.56796,1.18125 0.75027,1.95911 0.46372,3.37367 -0.0839,0.41394 -0.6938,1.7573 -0.89959,2.67781 -0.13567,0.60688 0.13543,0.88631 0.028,1.10465 -0.68013,1.38226 -0.85951,0.68912 -1.42722,1.53162 -0.82001,1.21692 -0.9135,2.26072 -0.9135,2.26072 -2.62487,0.21736 -5.43031,0.27039 -8.01047,-0.13519 -0.21074,-2.67946 -0.3887,-0.7924 -1.01165,-2.3228 z"
        self.svg.path(path, {'style': style_white_fill})
        paths = [
            # Left eye
            "m 101.77982,284.55725 c 0.004,0.96632 0.45036,2.0941 1.39212,2.05041 0.94176,-0.0437 1.37747,-1.03396 1.36409,-2.0552 -0.0134,-1.02124 -0.49476,-1.92637 -1.40364,-1.94387 -0.90889,-0.0175 -1.3563,0.98234 -1.35257,1.94866 z",
            # Nose
            "m 100.03721,289.11908 c -0.42377,-0.36462 -0.23973,-1.10247 0.38785,-2.14089 0.0843,-0.13938 0.41827,-0.11168 0.51275,0.021 0.6035,0.84762 0.67744,2.01011 0.33563,2.12543 -0.234,0.079 -0.36489,-0.0838 -0.58623,-0.37016 -0.23681,0.29673 -0.365,0.60983 -0.65,0.36461 z",
            # Right eye
            "m 96.79629,284.49096 c 0.004,0.96632 0.38407,2.09409 1.34792,2.0725 0.96385,-0.0216 1.44376,-1.01186 1.43038,-2.0331 -0.0134,-1.02124 -0.45057,-1.90429 -1.33735,-1.94387 -0.88677,-0.0396 -1.44468,0.93815 -1.44095,1.90447 z",
            # Outline
            "m 100.65625,276.60942 c -0.85308,-0.0126 -2.0994,0.19349 -2.75586,0.39844 -1.86941,0.58365 -2.83514,1.247 -3.88476,2.72657 -0.44,0.76902 -1.06058,1.73123 -0.99181,3.39023 0.058,1.39821 1.39651,3.32249 1.12006,4.24558 -0.10938,0.36524 -0.0785,0.6196 0.30526,1.01848 1.20238,1.37095 1.90562,2.41351 1.91019,3.23239 -0.008,0.23436 0.045,0.42743 0.24414,0.44141 2.80929,0.37265 5.12783,0.29481 7.9453,0.08 0.21185,-0.008 0.29297,-0.1875 0.29297,-0.39063 0.19662,-1.54374 0.9314,-2.00835 1.9863,-3.07614 0.28784,-0.27357 0.62427,-0.65332 0.29883,-1.33789 -0.23102,-0.48596 0.92245,-2.47356 1.06055,-3.55859 0.14911,-1.17151 -0.15793,-2.26393 -0.5293,-3.10157 -0.88762,-1.84613 -1.94831,-2.7592 -3.86523,-3.52929 -0.93109,-0.37405 -2.26053,-0.52603 -3.13664,-0.53899 z m -0.043,0.46285 c 1.09915,0.0507 2.08553,0.14066 3.01172,0.51567 1.72448,0.69823 2.70048,1.44439 3.55078,3.21288 0.36939,0.76826 0.69557,1.73619 0.54493,2.99415 -0.2428,1.03053 -1.05425,2.50934 -1.02149,3.02539 0,0 -0.0861,-0.15602 -0.21289,-0.41601 -0.18281,-0.37493 -0.21233,-0.57486 -0.27149,-1.11328 -0.0268,-0.24371 -0.29862,-0.32626 -0.29296,-0.22657 0.0416,0.73466 0.10464,1.0742 0.24804,1.5918 0.10978,0.39625 0.43528,0.73247 0.46485,1.14258 0.009,0.12977 -0.0461,0.34222 -0.13868,0.43359 -1.26028,1.19619 -2.08802,2.47382 -2.17578,3.43356 -2.51398,0.3381 -4.97436,0.2315 -7.47461,-0.0937 -0.14518,-1.85044 -1.10626,-2.38298 -2.14453,-3.76949 -0.18886,-0.25326 0.28842,-0.85838 0.40236,-1.26172 0.1228,-0.43469 0.32716,-0.88898 0.22258,-1.5508 -0.0245,-0.15513 -0.22882,0.13848 -0.28125,0.41603 -0.11651,0.61664 -0.28224,0.85172 -0.51435,1.37048 -0.0721,-1.12896 -0.74529,-2.04641 -0.91036,-3.0504 -0.28321,-1.72251 0.14892,-2.51897 0.80286,-3.6236 0.72882,-1.23113 1.99799,-2.15947 3.63348,-2.63984 0.72356,-0.21252 1.87504,-0.42215 2.55679,-0.39072 z",
            # Teeth Separators
            "m 97.6294,290.74837 c 0.0146,1.13412 -0.0956,1.27773 -0.0956,1.27773 l 0.61964,-0.048 c 0,0 -0.0652,-0.2374 -0.0975,-1.11199 -0.005,-0.1474 -0.42845,-0.26523 -0.42655,-0.11774 z",
            "m 99.05443,290.95071 c 0.0466,0.86236 -0.0404,1.24458 -0.0404,1.24458 l 0.53125,-0.0811 c 0,0 -0.054,-0.23486 -0.0422,-1.1341 0.002,-0.1427 -0.40962,-0.22457 -0.4376,-0.0846 z",
            "m 100.4009,290.99111 c 0.0202,0.9611 -0.0846,1.25563 -0.0846,1.25563 l 0.60859,-0.11428 c 0,0 -0.11429,-0.11132 -0.0532,-1.12305 0.0103,-0.17158 -0.47427,-0.19016 -0.47066,-0.0183 z",
            "m 101.7947,291.02458 c 0.0146,1.13412 -0.0957,1.1562 -0.0957,1.1562 l 0.61964,-0.048 c 0,0 -0.0653,-0.24845 -0.0974,-1.12304 -0.005,-0.1474 -0.42845,-0.13265 -0.42655,0.0148 z",
            "m 103.054,291.017 c 0.0466,0.86236 -0.0625,1.112 -0.0625,1.112 l 0.53125,-0.0811 c 0,0 -0.054,-0.16857 -0.0422,-1.06781 0.002,-0.1427 -0.43425,-0.1056 -0.42655,0.0369 z",
        ]
        for path in paths:
            self.svg.path(path, {'style': style_black_fill})

        self.svg.end_group()  # end skull

        self.svg.end_group()  # end skull and crossbones
    ...
    def draw_port_circle(self, id='', transform=''):
        ...
        self.svg.start_group({'transform': "translate(168.55655,-76.089882)"})
        ...
        path = "m 117.61371,135.33305 -14.27687,-7.03247 -5.122553,15.06798 -5.122556,-15.06798 -14.276867,7.03247 7.032478,-14.27686 -15.067982,-5.12256 15.067981,-5.12255 -7.032477,-14.276871 14.276866,7.032481 5.122557,-15.067985 5.122553,15.067985 14.27687,-7.032481 -7.03248,14.276861 15.06798,5.12256 -15.06798,5.12256 z"
        self.svg.path(path, {'style': style_black_fill})
        ...
        self.svg.path(path, {'style': style_black_fill, 'transform': rotate})
        ...
        self.svg.circle(266.77084, 39.843746, 19, {'style': circle_style})
        
        self.svg.end_group()
    
    def draw_small_port(self, id='', transform=''):
        self.svg.start_group({'id': id, 'transform': transform})
        ...
        #path = "m 117.61371,135.33305 -14.27687,-7.03247 -5.122553,15.06798 -5.122556,-15.06798 -14.276867,7.03247 7.032478,-14.27686 -15.067982,-5.12256 15.067981,-5.12255 -7.032477,-14.276871 14.276866,7.032481 5.122557,-15.067985 5.122553,15.067985 14.27687,-7.032481 -7.03248,14.276861 15.06798,5.12256 -15.06798,5.12256 z"
        path = "m 70.414002,125.63334 -7.13844,-3.51623 -2.561272,7.53399 -2.56128,-7.53399 -7.138434,3.51623 3.516239,-7.13843 -7.533989,-2.56128 7.533989,-2.56127 -3.516239,-7.13844 7.138434,3.51624 2.56128,-7.53399 2.561272,7.53399 7.13844,-3.51624 -3.51624,7.13843 7.53399,2.56128 -7.53399,2.56128 z"
        self.svg.path(path, {'style': style_black_fill})

        #rotate = "matrix(0.92387953,-0.38268343,0.38268343,0.92387953,-92.322534,69.370912)"
        #path = "m 174.56986,132.36623 -10.89361,-3.15815 -5.51643,9.91029 -5.46979,-9.9361 -10.90834,3.10693 3.15816,-10.89362 -9.91029,-5.51642 9.9361,-5.4698 -3.10693,-10.908329 10.89361,3.158149 5.51643,-9.910281 5.46979,9.936101 10.90834,-3.106932 -3.15816,10.893612 9.91029,5.51643 -9.9361,5.46979 z"
        path = "m 71.413822,120.395 -5.63648,0.62553 -0.652,5.63348 -4.427907,-3.54328 -4.44451,3.52243 -0.625525,-5.63648 -5.633478,-0.652 3.543279,-4.42791 -3.522433,-4.44451 5.636477,-0.62553 0.652009,-5.63347 4.427904,3.54328 4.444514,-3.52244 0.62552,5.63648 5.63348,0.65201 -3.54328,4.4279 z"
        #self.svg.path(path, {'style': style_black_fill, 'transform': rotate})
        self.svg.path(path, {'style': style_black_fill})
        ...
        circle_style = "opacity:1;fill:#ffffff;fill-opacity:1;stroke:#000000;stroke-width:1.79999995;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-dasharray:none;stroke-dashoffset:0;stroke-opacity:1"
        self.svg.circle(229.27084, 39.843746, 9, {'style': circle_style})
    ...
    def load_data(self, name):
        ...
        if ''.join(sorted(target_resources)) != ''.join(sorted(found_r)):
            ...
            print('Found resources:', [x[0] for x in resources])
    ...
    def process_png(self, name):
        ...
        if self.options['bleed']:
            cmd.append("--export-width=822")
            cmd.append("--export-height=822")
    ...
    def gen(self):
        ...
        for name in sorted(card_info.keys()):
            ...
            if label == "Unused":
                ...
                if label != "The Juven Isles":
                    error(f'Incorrect name for start card: {label}')
        ...
        for name in sorted(card_info.keys()):
            ...
            for r in res:
                ...
                if r[0] == 'p':
                    ...
                    has_neutral_port = True
                else:
                    res_count[r] += 1
                    rr.append(r)

            if has_neutral_port:
                neutral_port_count += 1
                for r in rr:
                    neutral_port_res_count[r] += 1
